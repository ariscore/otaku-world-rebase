...
class MangaPublishingStatusChips extends StatelessWidget {
  const MangaPublishingStatusChips({super.key, required this.selectedStatuses});
    ...
    return CustomChips(
      ...
      chipList: animeStatuses.map((s) {
        ...
        return CustomChoiceChip(
          ...
          onSelected: (status) {
            bloc.add(SelectPublishingStatus(status));
          },
          onUnselected: (status) {
            bloc.add(UnselectPublishingStatus(status));
          },
        );
      }).toList(),
    );
