...
class GenreCard extends StatefulWidget {
  ...
  final Fragment$UserStatistics$genres? genre;
  final int index;
...
class _GenreCardState extends State<GenreCard>
  ...
  Widget build(BuildContext context) {
    super.build(context);
    if (widget.genre == null) return const SizedBox();
    final client = context.read<GraphqlClientCubit>().getClient()!;

    return BlocProvider(
      key: UniqueKey(),
      create: (context) => MediaPostersBloc(idIn: widget.genre!.mediaIds)
        ..add(
          LoadData(client),
        ),
      child: Container(
        ...
        child: Column(
          children: [
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                _buildStat(context, count: widget.genre!.count, label: 'Count'),
                _buildSeparator(),
                widget.type == Enum$MediaType.ANIME
                    ? _buildStat(
                        context,
                        count: FormattingUtils.formatMinutes(
                          widget.genre!.minutesWatched,
                        ),
                        label: 'Time Watched',
                      )
                    : _buildStat(
                        context,
                        count: widget.genre!.chaptersRead,
                        label: 'Chapters Read',
                      ),
                _buildSeparator(),
                _buildStat(
                  context,
                  count: '${formatDouble(widget.genre!.meanScore)}%',
                  label: 'Mean Score',
                ),
              ],
            ),
